/*
 * Problem it Solves: Computes Shortest Linear Recurrence for sequence modulo any number (Composite/Prime) using Reeds-Sloane algorithm.
 * Usage: LinearRecurrence(s, mod, is_prime).solve(n).
 * Input/Output: Sequence s, mod -> k-th term.
 * Time Complexity: O(N^2) or O(N^2 log N) depending on implementation.
 * Tweaks: Works for composite moduli.
 * Other Use Cases: Decoding BCH codes (original use).
 * Warnings: Very complex algorithm.
 */
...
struct LinearRecurrence {
        ...
        for (size_t i = 1, m = 1; i < s.size(); ++i, m++) {
            ...
            if (2 * (A.size() - 1) <= i) {
                ...
                for (size_t j = 0; j < B.size(); ++j) {
                    A[j + m] = (A[j + m] - coef * B[j] % mod + mod) % mod;
                }
                B = temp, b = d, m = 0;
            } else {
                extend(A, B.size() + m);
    ...
    static ll crt(const vec &c, const vec &m) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            ans = (ans + (__int128)tm * x % M * c[i] % M) % M;
        }
        return (ans + M) % M;
    }
    ...
    static vec ReedsSloane(const vec &s, ll mod) {
        ...
        auto prime_power = [&] (const vec &s, ll mod, ll p, ll e) {
            ...
            for (size_t k = 1; k < s.size(); ++k) {
                ...
                for (int o = 0; o < e; ++o) {
                    ll d = 0;
                    for (size_t i = 0; i < a[o].size() && i <= k; ++i) d = (d + a[o][i] * s[k - i]) % mod;
                    if (d == 0) { t[o] = 1, u[o] = e; } 
                    else {
                        for (u[o] = 0, t[o] = d; t[o] % p == 0; t[o] /= p, ++u[o]);
                        int g = e - 1 - u[o];
                        if (L(a[g], b[g]) == 0) {
                            extend(bn[o], k + 1); bn[o][k] = (bn[o][k] + d) % mod;
                        } else {
                            ll coef = t[o] * inverse(to[g], mod) % mod * pw[u[o] - uo[g]] % mod;
                            int m = k - r[g];
                            extend(an[o], ao[g].size() + m); extend(bn[o], bo[g].size() + m);
                            for (size_t i = 0; i < ao[g].size(); ++i) an[o][i + m] = (an[o][i + m] - coef * ao[g][i] % mod + mod) % mod;
                            while (an[o].size() && an[o].back() == 0) an[o].pop_back();
                            for (size_t i = 0; i < bo[g].size(); ++i) bn[o][i + m] = (bn[o][i + m] - coef * bo[g][i] % mod + mod) % mod;
        ...
        vector<vec> as; size_t n = 0;
        for (auto &&x: fac) {
            ll mod, p, e; vec a, b;
            tie(mod, p, e) = x;
            auto ss = s; for (auto &&v: ss) v %= mod;
            tie(a, b) = prime_power(ss, mod, p, e);
            as.emplace_back(a); n = max(n, a.size());
        }
    ...
    ll solve(ll n) {
        ...
        vec v(m), u(m << 1);
        ll msk = !!n;
        for (ll len = n; len > 1; len >>= 1) msk <<= 1;
        v[0] = 1 % mod;
...
int main() {
    ...
    while (t--) {
        ...
        LinearRecurrence reeds_sloane(v, mod, 0); // 0 for composite
        cout << reeds_sloane.solve(2 * k) << '\n';
    }
}
