/*
 * Problem it Solves: Computes Discrete Root (x^k = a mod m) using BSGS.
 * Usage: discrete_root(k, a, m) returns one solution.
 * Input/Output: k, a, m (integers) -> x (integer) or -1.
 * Time Complexity: O(sqrt(m) * log(m)).
 * Tweaks: Can return all solutions (commented out).
 * Other Use Cases: Solving polynomial congruences x^k - a = 0.
 * Warnings: m must be prime (or have primitive root).
 */
...
int discrete_root(int k, int a, int m) {
    if (a == 0) return 0; // corrected from 1 to 0 (0^k=0)
