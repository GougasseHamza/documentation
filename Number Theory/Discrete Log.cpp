/*
 * Problem it Solves: Computes Discrete Logarithm (minimum x such that a^x = b mod m).
 * Usage: discrete_log(a, b, m) for gcd(a, m)=1. discrete_log_noncoprime(a, b, m) for general case.
 * Input/Output: a, b, m (integers) -> x (integer) or -1 if no solution.
 * Time Complexity: O(sqrt(m)).
 * Tweaks: Uses gp_hash_table for O(1) average lookup.
 * Other Use Cases: Breaking simple cryptographic schemes (like Diffie-Hellman with small params).
 * Warnings: Memory usage can be high for large m due to hash table.
 */
