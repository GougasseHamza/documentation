/*
 * Problem it Solves: Computes K-th root modulo P (x^k = a mod p) efficiently.
 * Usage: discrete_root(k, a, p) returns one solution x.
 * Input/Output: k, a, p (long long) -> x (long long) or -1.
 * Time Complexity: O(P^(1/4)) or faster probabilistic.
 * Tweaks: Logic handles complex cases where P-1 has small factors.
 * Other Use Cases: RSA decryption if private key is unknown but modulus factors are known (unlikely).
 * Warnings: P must be prime.
 */
