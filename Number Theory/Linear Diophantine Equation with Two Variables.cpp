/*
 * Problem it Solves: Finds number of solutions to ax + by + c = 0 in given ranges.
 * Usage: find_all_solutions(a, b, c, minx, maxx, miny, maxy).
 * Input/Output: coeffs, ranges (long long) -> count (long long).
 * Time Complexity: O(log(min(a, b))).
 * Tweaks: Handles edge cases like a=0 or b=0.
 * Other Use Cases: Integer points on a line segment.
 * Warnings: Pay attention to signs (ax + by + c = 0 vs ax + by = c).
 */
