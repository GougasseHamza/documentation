/*
 * Problem it Solves: Sieve of Eratosthenes optimized for large N (up to 1e9) using segmentation and wheel factorization.
 * Usage: sieve(N) returns vector of primes.
 * Input/Output: N (int) -> primes (vector<int>).
 * Time Complexity: O(N log log N) but highly optimized constant factor.
 * Tweaks: Q and L parameters tune the cache efficiency.
 * Other Use Cases: Generating primes up to 10^9 quickly.
 * Warnings: High complexity implementation (Min_25's sieve).
 */
