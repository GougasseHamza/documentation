/*
 * Problem it Solves: Finds the smallest primitive root modulo p.
 * Usage: primitive_root(p) returns g.
 * Input/Output: p (int) -> g (int).
 * Time Complexity: O(ans * log p * log(phi(p))).
 * Tweaks: p must be prime or prime power (except 2^k, k>=3).
 * Other Use Cases: Discrete log base, NTT roots.
 * Warnings: Returns -1 if no primitive root exists.
 */
