/*
 * Problem it Solves: Computes nCr % m for any m (not necessarily prime).
 * Usage: ncr(n, r, m) returns nCr % m.
 * Input/Output: n, r (long long), m (int) -> nCr (long long).
 * Time Complexity: O(m log m + log n).
 * Tweaks: Uses CRT and Lucas theorem generalization (Lucas is for prime p, here generalized).
 * Other Use Cases: Combinatorics with arbitrary modulus.
 * Warnings: Sieve precomputation required.
 */
...
// C(n, r) modulo p^k
int ncr_pk(ll n, ll r, int p, int k) {
...
int ncr(ll n, ll r, int m) {
    ...
    while (m > 1) {
        ...
        ans = CRT(ans.first, ans.second, ncr_pk(n, r, p, k), cur);
    }
    return ans.first;
}
