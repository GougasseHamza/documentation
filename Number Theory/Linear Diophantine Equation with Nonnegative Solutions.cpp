/*
 * Problem it Solves: Counts number of k in [l, r] such that sum(a[i] * x[i]) = k has non-negative solutions.
 * Usage: LDE solver(a); solver.count(l, r).
 * Input/Output: a (vector), l, r (long long) -> count (long long).
 * Time Complexity: O(min(a) * n * log(min(a))).
 * Tweaks: Uses Dijkstra to find shortest path in modulo graph.
 * Other Use Cases: Coin problem (Frobenius Coin Problem variant).
 * Warnings: a[i] must be positive.
 */
