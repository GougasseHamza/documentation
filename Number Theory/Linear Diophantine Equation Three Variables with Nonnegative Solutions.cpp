/*
 * Problem it Solves: Counts non-negative solutions to ax + by + cz = n.
 * Usage: yo(a, b, c, n) returns the count.
 * Input/Output: a, b, c, n (integers) -> count (integer).
 * Time Complexity: O(log(max(a, b, c))).
 * Tweaks: Requires __int128 for intermediate calculations.
 * Other Use Cases: Change making problem (for 3 coins).
 * Warnings: a, b, c must be positive.
 */
...
T count(T a, T b, T c, T n) {
    assert(__gcd((long long)a, (long long)b) == 1);
    assert(__gcd((long long)b, (long long)c) == 1);
    assert(__gcd((long long)a, (long long)c) == 1);
...
T yo(T a, T b, T c, T n) {
    T g = __gcd(__gcd((long long)a, (long long)b), (long long)c);
    if (n % g != 0) return 0;
    a /= g; b /= g; c /= g; n /= g;
    T g1 = __gcd((long long)b, (long long)c), g2 = __gcd((long long)c, (long long)a), g3 = __gcd((long long)a, (long long)b);
