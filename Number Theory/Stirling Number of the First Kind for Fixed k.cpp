/*
 * Problem it Solves: Computes Stirling Numbers of the First Kind S1(n, k) for fixed k and range of n.
 * Usage: stirling(n, k) returns vector of S1(i, k) for 0 <= i < n.
...
vector<int> stirling(int n, int k) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        cur = 1LL * cur * (i + 1) % MOD;
        ans[i] = 1LL * ans[i] * power(f, MOD - 2) % MOD;
    }
    return ans;
}
