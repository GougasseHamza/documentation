/*
 * Problem it Solves: Solves system of 2 linear equations with N unknowns over integers.
 * sum(x[i] * a[i]) = p
 * sum(x[i] * b[i]) = q
 * Usage: add(a, b) to add terms, can(p, q) to check existence.
 * Input/Output: queries.
 * Time Complexity: O(log val) per update/query.
 * Tweaks: Maintains basis (a, b, coeff) for the solution space.
 * Other Use Cases: Lattice problems.
 * Warnings: Global variables used.
 */
